...
namespace ChildCareManagementSystem
{
    public partial class card : Form
    {
        ...
        public card(int i, int tk, int ci, int pa)
        {
            child_id = ci;
            this.id = i;
            this.taka = tk;
        ...
        {
            string card_num = textBox1.Text.Trim();
            string card_pass = textBox2.Text.Trim();

            bool flag = true;

            if (card_num.Length != 12) { flag = false; }
            else
            {
                for (int i = 0; i < 12; i++)
                {
                    if ((card_num[i] < '0') || (card_num[i] > '9')) { flag = false; }
                }
            }
            if (card_pass.Length != 5) { flag = false; }
            if (!flag)
            {
